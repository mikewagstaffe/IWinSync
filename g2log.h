...
// Modified As DEBUG was already defined in the project --M.Wagstaffe
const int G2L_DEBUG = 0, G2L_INFO = 1, G2L_WARNING = 2, G2L_FATAL = 3;
static const std::string k_fatal_log_expression = ""; // using LogContractMessage but no boolean expression
static int  g_iLoggingLevel; //Added M.Wagstafffe To create conditional level logging
...
//Modified M.Wagstaffe To Accoiunt For Renamed Constants
// BELOW -- LOG stream syntax
#define G2_LOG_G2L_DEBUG  g2::internal::LogMessage(__FILE__,__LINE__,__PRETTY_FUNCTION__,"DEBUG")
#define G2_LOG_G2L_INFO  g2::internal::LogMessage(__FILE__,__LINE__,__PRETTY_FUNCTION__,"INFO")
#define G2_LOG_G2L_WARNING  g2::internal::LogMessage(__FILE__,__LINE__,__PRETTY_FUNCTION__,"WARNING")
#define G2_LOG_GTL_FATAL  g2::internal::LogContractMessage(__FILE__,__LINE__,__PRETTY_FUNCTION__,k_fatal_log_expression)

//Modified M Wagstaffe - Added if(level >= g_iLoggingLevel) to make logging conditional on current set level
// LOG(level) is the API for the stream log
#define LOG(level)				\
	if(level >= g_iLoggingLevel) \
		G2_LOG_##level.messageStream()

//Modified M Wagstaffe - Added && level >= g_iLoggingLevel to log condition to make logging conditional on current set level
// conditional stream log
#define LOG_IF(level, boolean_expression)						 \
  if(true == boolean_expression  && level >= g_iLoggingLevel )   \
    G2_LOG_##level.messageStream()	
...
#define G2_LOGF_G2L_INFO     g2::internal::LogMessage(__FILE__, __LINE__, __PRETTY_FUNCTION__,"INFO")
#define G2_LOGF_G2L_DEBUG    g2::internal::LogMessage(__FILE__, __LINE__, __PRETTY_FUNCTION__,"DEBUG")
#define G2_LOGF_G2L_WARNING  g2::internal::LogMessage(__FILE__, __LINE__, __PRETTY_FUNCTION__,"WARNING")
#define G2_LOGF_G2L_FATAL    g2::internal::LogContractMessage(__FILE__, __LINE__, __PRETTY_FUNCTION__,k_fatal_log_expression)
